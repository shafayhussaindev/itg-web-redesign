...
import { solutionDetail as copy } from '@/content/tier2/solutions/solution-detail.js';
      ...
      <main id="solution-main" className="solution-detail solution-category" tabIndex={-1}>
        ...
        <section className="sd-section sd-ice" aria-labelledby="related-title">
          <div className="section-container">
            ...
            <div className="sd-related" style={{ "--related-cols": related.length } as React.CSSProperties}>{related.map(item => <a href={`/${item.id}`} key={item.id} className="sd-related-card"><img src={item.image} alt="" loading="lazy" /><div><h3>{item.name}</h3><span>{copy.relatedLink}<ArrowRight size={17} /></span></div></a>)}</div>
          </div>
        </section>
