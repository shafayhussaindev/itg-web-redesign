import { industryFeatures } from '@/content/tier1/industries.js';
import { useInView } from '@/pages/tier1/shared/useInView.js';
 ...
 * ITG platform, its industries (five since Sept 2026).
 *
 * It carries NO written content of its own. The nodes are the industry
  ...
  return (
    <section className="ind-eco-band" aria-hidden="true">
      ...
      {/* Phone fallback: the SVG's two-sided fan-out is unreadable under
          ~900px, so CSS hides it and shows this instead. Same names. */}
...
/* Geometry. Node chips down each side of a central platform: the first half
   (rounded up) on the left, the rest on the right, each column centred on
   the hub. ROW_GAP is the distance between chips in a column. */
const W = 1080;
const CHIP_W = 300;
const CHIP_H = 62;
const ROW_GAP = 160;
...
/* One cycle for every pulse: travel, then a pause before the next run.
   Shared by every branch so they cannot drift apart. */
...
function EcosystemDiagram({ active }) {
  ...
  const half = Math.ceil(industryFeatures.length / 2);
  const left = industryFeatures.slice(0, half);
  const right = industryFeatures.slice(half);
  // Top edge of chip i in a column of n, centred vertically on the hub.
  const rowY = (i, n) => HUB_CY - CHIP_H / 2 + (i - (n - 1) / 2) * ROW_GAP;
      ...
      {sides.map((side) =>
        side.items.map((item, i) => (
          <path
            ...
            d={branch(side.edgeX, rowY(i, side.items.length) + CHIP_H / 2, side.hubX)}
          />
        ))
      )}

      {/* One pulse per branch, all on a single clock: identical begin
        ...
        side.items.map((item, i) => (
          <circle key={`p${side.key}${item.id}`} r="0" fill="#A8C6EA" className="ind-eco-pulse">
            <animateMotion
              ...
              keyPoints="0;1;1"
              keyTimes="0;0.52;1"
              path={branch(side.edgeX, rowY(i, side.items.length) + CHIP_H / 2, side.hubX)}
            />
            <animate
              attributeName="r"
              dur={PULSE_DUR}
      ...
      {sides.map((side) =>
        side.items.map((item, i) => (
          ...
          >
            ...
            />
            <text x={side.cx} y={rowY(i, side.items.length) + CHIP_H / 2 + 5} textAnchor="middle" className="ind-eco-node-label">
              {item.name}
            </text>
          </g>
        ))
