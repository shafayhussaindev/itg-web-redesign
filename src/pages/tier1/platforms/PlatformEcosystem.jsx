...
/**
 * PlatformEcosystem — ITG Platform Core and its six platforms.
 *
 ...
 * divisor of the 12s master cycle, so the whole illustration is seamless.
 *
 * The 12s cycle: each of the six nodes activates in turn for 2s — glow up,
 ...
 *
 * Layout: the six nodes sit on a flat-topped hexagon around the core (radius
 * 260, centre 400,400). Lines run from the core's edge (86 out) to each node's
 * edge (54 in). To rename a node, change its `label` (\n breaks the line).
 *
...
/* Clockwise from top-left. Order = the order the nodes light up. */
const NODES = [
  {
    id: 'sourcing', label: 'Sourcing',
    x: 270, y: 174.8,
    lineStart: { x: 357, y: 325.5 }, lineEnd: { x: 297, y: 221.6 },
    labelAnchor: { x: 270, y: 105, align: 'middle' },
    icon: 'cart',
  },
  {
    id: 'supply-chain', label: 'Supply Chain',
    x: 530, y: 174.8,
    lineStart: { x: 443, y: 325.5 }, lineEnd: { x: 503, y: 221.6 },
    labelAnchor: { x: 530, y: 105, align: 'middle' },
    icon: 'truck',
  },
  {
    id: 'contract-lifecycle', label: 'Contract\nLifecycle',
    x: 660, y: 400,
    lineStart: { x: 486, y: 400 }, lineEnd: { x: 606, y: 400 },
    labelAnchor: { x: 725, y: 393, align: 'start' },
    icon: 'contract',
  },
  {
    id: 'supplier-info-risk-management', label: 'Supplier Info &\nRisk Management',
    x: 530, y: 625.2,
    lineStart: { x: 443, y: 474.5 }, lineEnd: { x: 503, y: 578.4 },
    labelAnchor: { x: 530, y: 705, align: 'middle' },
    icon: 'risk',
  },
  {
    id: 'product-lifecycle-management', label: 'Product Lifecycle\nManagement',
    x: 270, y: 625.2,
    lineStart: { x: 357, y: 474.5 }, lineEnd: { x: 297, y: 578.4 },
    labelAnchor: { x: 270, y: 705, align: 'middle' },
    icon: 'cycle',
  },
  {
    id: 'data-privacy', label: 'Data\nPrivacy',
    x: 140, y: 400,
    lineStart: { x: 314, y: 400 }, lineEnd: { x: 194, y: 400 },
    labelAnchor: { x: 75, y: 393, align: 'end' },
    icon: 'lock',
  },
];
...
const SLOT = 2;                     // seconds each node stays lit
const CYCLE = SLOT * NODES.length;  // the master loop: 12s for six nodes
...
function Icon({ type, size = 22 }) {
  ...
  switch (type) {
    case 'cart':
      /* Sourcing */
      return (<svg {...common}><circle cx="9" cy="20" r="1.4" /><circle cx="17" cy="20" r="1.4" /><path d="M2.5 3.5h2.6l2.3 11.2a1.5 1.5 0 0 0 1.5 1.2h8.4a1.5 1.5 0 0 0 1.5-1.2l1.5-7.2H6.2" /></svg>);
    case 'truck':
      /* Supply chain */
      return (<svg {...common}><path d="M2.5 6.5h11v10h-11z" /><path d="M13.5 10h4.2l3 3.4v3.1h-7.2" /><circle cx="7" cy="18" r="1.8" /><circle cx="17.5" cy="18" r="1.8" /></svg>);
    case 'contract':
      /* Contract lifecycle: a page with a signature line */
      return (<svg {...common}><path d="M6 2.8h8.5L19 7.3v13.9H6Z" /><path d="M14.5 2.8v4.5H19" /><path d="M9 11h7M9 14.5h7" /><path d="M9 18.5c1-1.2 1.8-1.2 2.4 0s1.4 1.2 2.4 0" /></svg>);
    case 'risk':
      /* Supplier info & risk: a shield with a check */
      return (<svg {...common}><path d="M12 2.8 19.5 6v5.6c0 4.6-3.2 8.2-7.5 9.6-4.3-1.4-7.5-5-7.5-9.6V6Z" /><path d="M8.8 12.2l2.3 2.3 4.3-4.6" /></svg>);
    case 'cycle':
      /* Product lifecycle: two arrows chasing round a circle */
      return (<svg {...common}><path d="M19.5 12a7.5 7.5 0 0 1-12.9 5.2" /><path d="M4.5 12a7.5 7.5 0 0 1 12.9-5.2" /><path d="M17.8 3.2v3.9h-3.9" /><path d="M6.2 20.8v-3.9h3.9" /></svg>);
    case 'lock':
      /* Data privacy */
      return (<svg {...common}><rect x="5" y="10.5" width="14" height="10" rx="2" /><path d="M8.2 10.5V7.6a3.8 3.8 0 0 1 7.6 0v2.9" /><circle cx="12" cy="15.5" r="1.3" /></svg>);
    default:
      return null;
  }
}
...
const STYLE = `
  ...
  /* Cyan tint that rides the master cycle: on for this node's 2s slot only. */
  ...
  /* Orbits carry evenly spaced marks, so a rotation of one mark-step is
     ...
     still lands exactly where it started at the end of the cycle. */
    ...
    return (
      <div ref={ref} className="itg-eco-wrap">
        ...
        <svg
          ...
          role="img"
          aria-label="The ITG platform core connected to six platforms: Sourcing, Supply Chain, Contract Lifecycle, Supplier Info and Risk Management, Product Lifecycle Management, and Data Privacy."
        >
          {defs}
          ...
          <circle r="3" className="itg-eco-particle" opacity="0">
            <animateMotion dur={`${CYCLE}s`} repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="linear">
              <mpath href="#itg-eco-spine" />
            </animateMotion>
            <animate attributeName="opacity" values="0;0.8;0.8;0" keyTimes="0;0.12;0.88;1" dur={`${CYCLE}s`} repeatCount="indefinite" />
          </circle>
