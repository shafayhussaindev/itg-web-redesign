...
import PlatformEcosystem from './PlatformEcosystem.jsx';
...
/* Material Symbols. The philosophy principles use the short keys below; the
   platform cards give a Material Symbols name directly, which passes through. */
const PP_GLYPHS = {
  check: 'check',
  plug: 'power',
  ...
  return (
    ...
    >
      ...
      <div className="pp-cat-body">
        ...
        <a className="pp-cat-cta" href={`/${cat.id}`}>
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      setGeo({
        ...
        // stop at the 2×2 block: an odd last card spans the full row below it
        bottom: Math.max(cards[2].bottom, cards[3].bottom) - base.top,
    ...
    <Tier1Route title="Platforms">
      <div>
        <main>
          ...
          <section className="pp-hero">
            ...
            <div className="pp-hero-inner">
              <div className="pp-hero-glass">
                <h1>{platformsHero.title}</h1>
                <p className="pp-hero-sub">
                  <Accented text={platformsHero.subtitle} accent={platformsHero.subtitleAccent} />
                </p>
                {platformsHero.body.map((line) => (
                  <p className="pp-hero-body" key={line}>{line}</p>
                ))}

                <div className="pp-hero-ctas">
                  <a className="btn-cyan" href={platformsHero.primaryCta.href}><CtaLabel>{platformsHero.primaryCta.label}</CtaLabel></a>
                  <a className="btn-outline-navy" href={platformsHero.secondaryCta.href}>
                    {platformsHero.secondaryCta.label}<span className="btn-arrow">→</span>
                  </a>
                </div>
              </div>
              ...
              <aside className="pp-hero-panel">
                <h2>{platformsHero.panel.heading}</h2>
                <ul>
                  {platformsHero.panel.items.map((item) => (
                    <li key={item}>
          ...
          {/* ---------- 3 · THE PLATFORMS ---------- */}
          <section ref={categoriesRef} className="pp-cats" id="platforms">
            <div className="pp-section-head">
              <h2>{platformsHeading}</h2>
            </div>
            <CategoryDeck cats={platforms} />
          </section>
          ...
          {/* ---------- 5 · HOW ITG PLATFORMS WORK TOGETHER ---------- */}
          ...
          <section className="pp-cta pp-grid-field">
            <div>
              <h2>{platformsCta.heading}</h2>
              <div className="pp-cta-btns">
                <a className="btn-cyan" href={platformsCta.primary.href}><CtaLabel>{platformsCta.primary.label}</CtaLabel></a>
                <a className="btn-outline-navy" href={platformsCta.secondary.href}>
                  {platformsCta.secondary.label}<span className="btn-arrow">→</span>
                </a>
              </div>
            </div>
          </section>
